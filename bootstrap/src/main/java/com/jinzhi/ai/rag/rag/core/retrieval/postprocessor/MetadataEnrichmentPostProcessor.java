...
import com.jinzhi.ai.rag.knowledge.service.impl.ChunkMetadataResolver;
...
public class MetadataEnrichmentPostProcessor implements SearchResultPostProcessor {
    ...
    public List<RetrievedChunk> process(List<RetrievedChunk> chunks,
        ...
        Map<String, ChunkMetadataResolver.ChunkMeta> metaById = chunkMetadataResolver.resolve(chunkIds);

        // 1）按 chunkId 富化：向量 / 关键词证据的 chunk.id 即向量库主键，回表补齐 docId / 序号 / 标题
        // 原地富化，保持相关性顺序不变
        for (RetrievedChunk chunk : chunks) {
            ChunkMetadataResolver.ChunkMeta meta = metaById.get(chunk.getId());
            if (meta == null) {
                continue;
            }
